...
- **Analyze:** `afs analyze` is read-only and reports detected project types, testing stacks, evidence, and the resolved install plan
...
- **Skills-only mode:** `afs add-agent --only-skills` manages only the skills tree; without `--force`, invalid existing skill files are preserved but omitted from the generated skills indexes
